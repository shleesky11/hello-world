    ...
    def control_driving(self, car_controls, sensing_info):
        ...
        if sensing_info.speed > 0 and self.st_collided == 0:
            if self.check_road_limit(sensing_info):
                if sensing_info.to_middle > 0:
                    if sensing_info.speed < 40:
                        ...
                        car_controls.steering = car_controls.steering - 0.15
                else:
                    if sensing_info.speed < 40:
                        car_controls.steering = car_controls.steering + 0.5
                    elif sensing_info.speed < 80:
            ...
            if len(sensing_info.track_forward_obstacles) > 0:
                middle_from_ob = abs(sensing_info.to_middle - sensing_info.track_forward_obstacles[0].get('to_middle'))
                dist_from_ob = sensing_info.track_forward_obstacles[0].get('dist')
                
                ob_mid = sensing_info.track_forward_obstacles[0].get('to_middle')
                if dist_from_ob < 30 and middle_from_ob < 2:
                    if abs(sensing_info.to_middle) < abs(ob_mid):
                        car_controls.steering = car_controls.steering - ob_mid/abs(ob_mid)*0.4
                    else:
                        car_controls.steering = car_controls.steering + ob_mid/abs(ob_mid)*0.4
                elif dist_from_ob < 70 and middle_from_ob < 2:
                    self.set_brake(car_controls, sensing_info, 0.2)
                    if ob_mid * sensing_info.to_middle > 0:
                        car_controls.steering = car_controls.steering - ob_mid/abs(ob_mid)*0.2
            ...
            #processing for car collision outside road
                ...
                if abs(sensing_info.to_middle) > self.half_road_limit:
                    ...
                    if self.prev_count == 0:
                        ...
                        self.prev_throttle = -1
                    car_controls.throttle = self.prev_throttle
                    if abs(sensing_info.moving_angle) < 30:
                        if sensing_info.to_middle < 0:
